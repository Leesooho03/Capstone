	...
	while (fileIter.hasNext()) {
	    ...
	    String tmp = mFile.getOriginalFilename();
            ...
            if (mFile.getSize() > 0) {
            	...
            	// 폴더 권한 오류 발생할수 있어 ServerSubPath 제거
//            	saveFile(mFile.getInputStream(), new File(EgovWebUtil.filePathBlackList(where+SEPERATOR+vo.getPhysicalName())));
            	
            	//리눅스 접근권한 없을때 추가
//            	Runtime.getRuntime().exec("chmod -R 755 " + EgovWebUtil.filePathBlackList(where + SEPERATOR + vo.getServerSubPath() + SEPERATOR + vo.getPhysicalName()));
        	list.add(vo);
            }
	}
...
//					saveFile(is, new File(EgovWebUtil.filePathBlackList(where + SEPERATOR + vo.getPhysicalName())));
					//리눅스 접근권한 없을때 추가
//	            	Runtime.getRuntime().exec("chmod -R 777 " + EgovWebUtil.filePathBlackList(where + SEPERATOR + vo.getServerSubPath() + SEPERATOR + vo.getPhysicalName()));
				} finally {
					if (is != null) {
						is.close();
					}
